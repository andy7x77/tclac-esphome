...
void tclacClimate::control(const ClimateCall &call) {
	...
	ESP_LOGW("TCL", "DBG10: past SWING block");
	
	// Расчет температуры
	if (call.get_target_temperature().has_value()) {
		ESP_LOGW("TCL", "DBG11: reading call.get_target_temperature().value()");
		target_temperature_set = 31-(int)call.get_target_temperature().value();
	} else {
		ESP_LOGW("TCL", "DBG12: target_temperature=%f", target_temperature);
		target_temperature_set = 31-(int)target_temperature;
	}
	ESP_LOGW("TCL", "DBG13: past TEMP block, about to takeControl()");
